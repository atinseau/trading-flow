...
export async function setupWorkflow(initial: InitialEvidence): Promise<SetupStatus> {
  ...
  setHandler(corroborateSignal, async (args) => {
    ...
    // **Race-fix**: mutate `state.{score,status}` BEFORE the persist await.
    // Concurrent signal handlers (priceCheck, review, or another corroborate
    // from a later tick) snapshot `state.score` at handler entry ; if we
    // mutated only after the await, they would all read the stale
    // pre-mutation value and clobber each other's writes. Mirrors the same
    // fix applied to `priceCheckSignal` and the pattern in `reviewSignal`.
