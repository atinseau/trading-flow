...
export function computeScalars(candles: Candle[]) {
  ...
  const recentH = equalPivots(
    swings.highs.filter((i) => i >= candles.length - RECENT),
    highs,
    TOLERANCE,
  );
  const recentL = equalPivots(
    swings.lows.filter((i) => i >= candles.length - RECENT),
    lows,
    TOLERANCE,
  );
  return {
...
export function computePriceLines(candles: Candle[]) {
  ...
  return [
    ...eh.map((e) => ({
      price: e.price,
      color: "rgba(255,235,59,0.6)" as const,
      style: 1 as const,
      title: `EQH×${e.indices.length}`,
    })),
    ...el.map((e) => ({
      price: e.price,
      color: "rgba(255,235,59,0.6)" as const,
      style: 1 as const,
      title: `EQL×${e.indices.length}`,
    })),
  ];
}
