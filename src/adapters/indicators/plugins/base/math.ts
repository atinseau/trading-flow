...
export function rsiSeriesAligned(closes: number[], period: number, n: number): (number | null)[] {
...
export function emaSeriesAligned(values: number[], period: number, n: number): (number | null)[] {
  const out: (number | null)[] = Array.from({ length: n }, () => null);
...
export function zScoreOfLast(series: number[], window: number): number {
...
export function vwapSeries(candles: Candle[]): number[] {
  const aligned = vwapSeriesAligned(candles);
  return aligned.filter((v): v is number => v !== null);
}

export function vwapSeriesAligned(candles: Candle[]): (number | null)[] {
...
export function bollingerSeriesAligned(
  closes: number[],
  period: number,
  stdMul: number,
): {
...
export function macdLast(
  ...
  const macdSeries = macdSeriesAligned(closes, fast, slow, signal);
...
export function macdSeriesAligned(
  closes: number[],
  fast: number,
  slow: number,
  signalPeriod: number,
  ...
  const fastEma = emaSeriesAligned(closes, fast, n);
  const slowEma = emaSeriesAligned(closes, slow, n);
  ...
  const signalSeries = emaSeriesAligned(macdValues, signalPeriod, n);
...
export function detectBosState(
  highs: number[],
  lows: number[],
  closes: number[],
...
export function pointOfControl(candles: Candle[], buckets: number): number {
...
export function rollingMaAligned(values: (number | null)[], period: number): (number | null)[] {
