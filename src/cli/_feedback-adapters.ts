...
export async function wireFeedbackActivitiesForCli(): Promise<FeedbackCliWiring> {
  const configPath = process.env.WATCHES_CONFIG_PATH ?? "config/watches.yaml";
  ...
  const watches = await loadWatchesConfig(configPath, { pool });
  if (watches === null) {
    await pool.end();
    throw new Error(
      `replay-feedback requires watches config (none found at ${configPath} and no DB watches)`,
    );
  }
  ...
  const llmProviders = buildProviderRegistry(watches, infra, llmUsageStore);
