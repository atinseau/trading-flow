...
const CLAUDE_MAX_MODELS: ComboOption[] = [
  {
    ...
    hint: "Le plus capable — analyse profonde, plus cher",
  },
  {
    value: "claude-sonnet-4-6",
    label: "claude-sonnet-4-6",
    hint: "Équilibre vitesse / qualité — défaut recommandé",
  },
  {
    value: "claude-haiku-4-5",
    label: "claude-haiku-4-5",
    hint: "Rapide et peu cher — bon pour le Reviewer",
  },
];
...
const OPENROUTER_MODELS: ComboOption[] = [
  ...
  },
  {
    ...
    hint: "Anthropic via OpenRouter — rapide",
  },
  { value: "openai/gpt-4o", label: "openai/gpt-4o", hint: "OpenAI" },
  {
    value: "openai/gpt-4o-mini",
    label: "openai/gpt-4o-mini",
    hint: "OpenAI — rapide et économique",
  },
...
const ROLES: { key: "detector" | "reviewer" | "finalizer"; label: string; help: string }[] = [
  {
    key: "detector",
    label: "Détecteur",
    help: "Tourne à chaque tick. Analyse le graphique frais et propose / renforce des setups. C'est le plus fréquent — privilégie un modèle rapide et raisonnable en coût.",
  },
  {
    key: "reviewer",
    label: "Reviewer",
    help: "Affine les setups vivants en croisant les données fraîches avec la mémoire des analyses passées. Choix typique : modèle léger (Haiku) — il tourne souvent.",
  },
  {
    key: "finalizer",
    label: "Finalizer",
    help: "Tourne UNE fois par setup quand le score atteint le seuil. Décision GO/NO_GO finale. Choix typique : modèle premium (Opus) — peu fréquent, donc le coût absolu reste bas.",
  },
];
...
function AnalyzerRow({
  ...
  const modelHint = (() => {
    if (provider === "claude_max") return "Modèles Anthropic uniquement (Claude Agent SDK).";
  ...
  const modelEmpty = (() => {
    ...
    if (provider === "openrouter") return "Tape un model ID (ex: anthropic/claude-3.5-sonnet).";
    return "Choisis d'abord un provider";
  })();
            ...
            <FormItem>
              ...
              <FormControl>
                <ComboInput
                  ...
                  placeholder={
                    modelOptions.length === 0 ? "Choisis un provider…" : "Choisir un modèle…"
                  }
    ...
    <div className="space-y-6">
      <div className="rounded-lg border bg-muted/30 p-4 text-sm space-y-2">
        ...
        <p className="text-muted-foreground">
          Chaque tick lance le <strong>Detector</strong>. Si un setup vit, le{" "}
          <strong>Reviewer</strong> le refine au tick suivant. Quand le score franchit le seuil, le{" "}
          <strong>Finalizer</strong> tranche en GO/NO_GO et envoie la notification.
        </p>
        <p className="text-muted-foreground">
          La liste de modèles s'adapte au provider choisi : <strong>claude_max</strong> n'expose que
          les modèles Anthropic ; <strong>openrouter</strong> accepte n'importe quel ID en saisie
          libre.
        </p>
      </div>
