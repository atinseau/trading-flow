      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-xl font-bold flex items-center gap-3">
            ...
            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${watch.enabled ? "bg-emerald-500/15 text-emerald-400" : "bg-zinc-500/15 text-zinc-400"}`}>
              {watch.enabled ? "Active" : "Pause"}
            </span>
          </h1>
          <p className="text-sm text-muted-foreground mt-1 font-mono">
            {cfg.asset.symbol} · {cfg.timeframes.primary} · {cfg.asset.source}
            {watch.state?.lastTickAt && (
              <> · dernier tick {fmtRelative(watch.state.lastTickAt)}</>
            )}
          </p>
        </div>
        <ConfirmAction
