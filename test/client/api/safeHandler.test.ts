import { ConflictError, NotFoundError, ValidationError, safeHandler } from "@client/api/safeHandler";
...
describe("safeHandler", () => {
  ...
  test("returns 500 on uncaught exceptions", async () => {
    const h = safeHandler(async () => { throw new Error("boom"); });
    const res = await h(new Request("http://x/y"));
  ...
  test("returns 409 on ConflictError", async () => {
    const h = safeHandler(async () => { throw new ConflictError("version mismatch"); });
    const res = await h(new Request("http://x/y"));
    expect(res.status).toBe(409);
  });

  test("returns 404 on NotFoundError", async () => {
    const h = safeHandler(async () => { throw new NotFoundError("missing"); });
    const res = await h(new Request("http://x/y"));
    expect(res.status).toBe(404);
  });

  test("returns 400 on ValidationError", async () => {
    const h = safeHandler(async () => { throw new ValidationError("bad input"); });
    const res = await h(new Request("http://x/y"));
    expect(res.status).toBe(400);
  });
});
